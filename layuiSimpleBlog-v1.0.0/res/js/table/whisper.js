...
layui.use('table', function () {
    ...
    table.render({
        elem: '#LAY_table_user'
        , url: baseUrl + '/whisper/getWhisperList' //数据接口
        , response: {
            statusCode: 200 //规定成功的状态码，默认：0
        }
        , cols: [[
            {field: 'content', title: '内容', width: 180, sort: true, fixed: true}
            , {field: 'classCode', title: '类别', width: 100}
            , {field: 'isShow', title: '是否显示', width: 180}
    ...
    table.on('tool(useruv)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                $.ajax({
                    url: baseUrl + "/whisper/deleteWhisper?id=" + data.id,
                    type: "GET",
    ...
    function EidtUv(data, value, index, obj) {
        ...
        $.ajax({
            url: baseUrl + "/whisper/updateWhisper?isShow=" + value + "&id=" + data.id,
            type: "GET",
